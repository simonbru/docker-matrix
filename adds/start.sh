...
configure_homeserver_yaml() {
	...
	awk -v TURNURIES="turn_uris: [\"turn:${SERVER_NAME}:3478?transport=udp\", \"turn:${SERVER_NAME}:3478?transport=tcp\"]" \
	    ...
	    -v PIDFILE="pid_file: /data/matrix-server.pid" \
	    -v DATABASE="database: \"/data/matrix-server.db\"" \
	    -v LOGFILE="log_file: \"/data/matrix-server.log\"" \
		...
		sub(/pid_file: \/matrix-server.pid/, PIDFILE);
		sub(/database: "\/matrix-server.db"/, DATABASE);
		sub(/log_file: "\/matrix-server.log"/, LOGFILE);
...
configure_log_config() {
	sed -i "s|.*filename:\s/matrix-server.log|    filename: /data/matrix-server.log|g" "/data/${SERVER_NAME}.log.config"
}
...
case $OPTION in
	...
	"diff")
		...
		generate_synapse_file /tmp/matrix-server.synapse.yaml
		diff -${DIFFPARAMS} /tmp/matrix-server.synapse.yaml /data/matrix-server.yaml
		;;
	...
	"generate")
		...
		generate_synapse_file /data/matrix-server.tmp
		echo "-=> configure some settings in matrix-server.yaml"
		configure_homeserver_yaml $turnkey /data/matrix-server.tmp

		mv /data/matrix-server.tmp /data/matrix-server.yaml

		echo "-=> configure some settings in ${SERVER_NAME}.log.config"
		configure_log_config

		echo ""
		echo "-=> you have to review the generated configuration file matrix-server.yaml"
		;;
